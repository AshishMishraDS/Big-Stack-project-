...
router.post("/register", (req, res) => {
    Person.findOne({ email: req.body.email})
        .then( person => {
            if (person) {
                return res.status(400).json({emailerror: "Email is allready register in our website"})
            } else {
...
router.post("/login", (req, res) => {
    ...
    const password = req.body.password;
     ...
     .then( person => {
         if(!person) {
             return res.status(404).json({emailerror : "User is not found with this email"});
         }
